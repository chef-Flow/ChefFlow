...
export default function CompartirSubRecetaModal({ subRecetaId, subRecetaNombre, onClose }: Props) {
  ...
  useEffect(() => {
    Promise.all([
      getContactosCompartir(),
      getSharesDeSubReceta(subRecetaId),
    ]).then(([c, s]) => {
      setContactos(c)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await compartirSubReceta(subRecetaId, email, { puedeVerPrecios, puedeVerProveedores })
    setLoading(false)
    ...
    const [updated, updatedContacts] = await Promise.all([
      getSharesDeSubReceta(subRecetaId),
      getContactosCompartir(),
    ])
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-slate-900">Compartir sub-receta</h2>
              <p className="text-xs text-slate-400 truncate max-w-[220px]">{subRecetaNombre}</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="relative">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm text-slate-800 placeholder:text-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-transparent"
                  required
                />
                {contactos.length > 0 && (
                  <button
            ...
            {success && !error && (
              <p className="text-xs text-green-700 bg-green-50 border border-green-100 rounded-lg px-3 py-2 flex items-center gap-1.5">
                <Check size={13} /> Sub-receta compartida correctamente
              </p>
            )}
            ...
            >
              ...
              {loading ? 'Enviando…' : 'Compartir sub-receta'}
            </button>
          </form>
